...
cont() {
    ...
    if [[ -z "$CONT_PATH" || -z "$MOUNT_PATH" ]]; then
        echo -e "\n${YELLOW}Usage:${RESET} cont-open ${CYAN}<container-dir> <mount-point>${RESET}\n"
        return 1
    else
        ...
        # Select container
        echo -e "\n══════════════════════════════════════════════════════"
        echo -e "${YELLOW}Select the LUKS container to mount:${RESET}"
        echo -e "══════════════════════════════════════════════════════\n"
        PS3=$'\nOption: '
    ...
    # Keyfile support
    echo -e "${BLUE}Searching for keyfile (eg. *.keyfile)...${NC}"
    KEYFILE=$(echo "$CONT_TARGET" | sed 's/\.[^.]*$/.keyfile/')

    if [[ -f "$KEYFILE" ]]; then
        echo -e "${CYAN}'$KEYFILE'${RESET} found."
        KEYFILE_FLAG="1"
    else
        echo -e "${YELLOW}Keyfile not found.${RESET}"
        read -p "Do you want to add an existing keyfile? [y/n]: " SEARCH_K_REPLY
        if [[ ! $SEARCH_K_REPLY =~ ^[Yy]$ ]]; then
            ...
            if [[ -f "$KEYFILE_LOC" ]]; then
                ...
                echo -e "\n${YELLOW}Keyfile '$KEYFILE_LOC' not found${RESET}.\nProceeding with password-only support."
            fi
        fi
    fi

    # Set container alias
    echo -e "\n${YELLOW}Create an alias to identify the working LUKS container.${RESET}\nAvoid names with empty spaces. Use hyphens or underscores for multiple words"
    ...
    # Set up loop device
    if [[ -z "$LOOP_NUM" ]]; then
        echo -e "\n${RED}Error:${RESET} Failed to set up loop device."
        return 1
    else
    ...
    if [[ -n "$KEYFILE_FLAG" ]]; then
        if ! sudo cryptsetup luksOpen "$LOOP_NUM" "$CONT_ALIAS" --key-file "$KEYFILE" &>/dev/null; then
            # Fallback if fails to open
            echo -e "${YELLOW}Falling back to opening with password...${RESET}"
            if ! sudo cryptsetup luksOpen "$LOOP_NUM" "$CONT_ALIAS" &>/dev/null; then
                echo -e "\n${RED}Error:${RESET} Failed to open LUKS container.\n"
    ...
    echo -e "${GREEN}Container mounted at '$MOUNT_PATH'${RESET}"
    echo -e "${GREEN}Write access enabled at '$MOUNT_PATH'${RESET}\n"
    echo -e "══════════════════════════════════════════════════════\n"
}
...
cont_close() {
    ...
    if [[ -z "$CONT_ALIAS2" ]]; then
        echo -e "\n${YELLOW}Usage:${RESET} cont_close ${CYAN}<container-alias>${RESET}\n"
        return 1
    fi
    ...
    if [[ -n "$MOUNT_PATH2" && -d "$MOUNT_PATH2" ]]; then
        ...
        echo -e "${YELLOW}Warning: No mount point found for '$CONT_ALIAS2'${RESET}"
    fi
    ...
    unset CONT_PATH MOUNT_PATH LOOP_NUM CONT_TARGET CONT_ALIAS KEYFILE_FLAG KEYFILE KEYFILE_LOC
