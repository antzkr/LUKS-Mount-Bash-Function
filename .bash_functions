...
cont_close() {
    ...
    # Detach loop device
    sudo losetup -d "$LOOP_NUM2" 2>/dev/null && echo -e "${GREEN}Loop device $LOOP_NUM2 detached${NC}" || echo -e "${YELLOW}Warning: Could not detach loop device (might already be detached)${NC}"

    # Unset environment variables if they were set by cont()
    unset CONT_FILE LOOP_NUM2 MOUNT_PATH CONT_ALIAS

    # Optionally unset the exported variables from cont()
    unset CONT_PATH MOUNT_PATH LOOP_NUM CONT_TARGET CONT_ALIAS KEYFILE

    echo -e "${BLUE}Container safely unmounted and closed${NC}\n"
}
